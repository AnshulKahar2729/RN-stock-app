...
import { StockSearch, StockOverview } from '../types/stock';

const fetchStockOverview = async (ticker: string): Promise<StockOverview> => {
  const response = await axios.get(
    `${config.BASE_URL}/stock/overview?symbol=${ticker}`,
  );
...
const fetchStockSearch = async (query: string): Promise<StockSearch[]> => {
  // URL is https://www.alphavantage.co/query?function=SYMBOL_SEARCH&keywords=AAPL&apikey=demo
  const response = await axios.get(
    `${config.BASE_URL}/stock/search?query=${query}`,
  );
...
const fetchTimeSeriesDaily = async (
  ...
  // URL is https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=AAPL&apikey=CYVRNP2JVC24IC81
  const response = await axios.get(
    `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${ticker}&apikey=${config.API_KEY}`,
  );

  console.log('response', response);

  // if (!response.data || Object.keys(response.data).length === 0) {
  //   throw new Error('No data found for time series daily');
  // }

  return response.data['Time Series (Daily)'];
};

export const useTimeSeriesDaily = (ticker: string) => {
  return useQuery({
